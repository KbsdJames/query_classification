            ...
            if (step + 1) % args.gradient_accumulation_steps == 0 or (
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if (
                        ...
                        dev_logs, do_save = evaluate_during_train(best_metric)
...
def evaluate(args, model, tokenizer, prefix="", part='dev'):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating", disable=args.no_tqdm):
            ...
            #     )  # XLM, DistilBERT, RoBERTa, and XLM-RoBERTa don't use segment_ids
            inputs.pop('token_type_ids')
            eval_input += tokenizer.batch_decode(inputs['input_ids'], skip_special_tokens=True)
            outputs = model(**inputs, return_dict=False)
        ...
        with open(output_eval_file, "w") as writer:
            ...
            for i in range(len(results['eval_input'])):
                writer.write("%s\t%s\t%s\n" % (results['eval_input'][i], results['preds'][i], results['out_label_ids'][i]))
            for key in sorted(result.keys()):
                if(key not in ['eval_input', 'preds', 'out_label_ids']):
